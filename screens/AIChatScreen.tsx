...
const AIChatScreen: React.FC<AIChatScreenProps> = ({ onBack }) => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { id: '1', senderId: aiUser.id, text: '您好，我是您的AI搭子助手，有什么可以帮助您的吗？无论是学习计划、赛事信息还是简历优化，我都可以提供帮助。', timestamp: '...' },
    ]);
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        if (!API_KEY) {
            const errorMsg: Message = { id: String(Date.now() + 1), senderId: aiUser.id, text: "错误：API 密钥未配置。", timestamp: '...'};
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100">
      <header className="sticky top-0 flex items-center p-3 bg-white/80 backdrop-blur-lg z-10 shadow-sm">
        ...
        <h2 className="text-lg font-bold text-gray-800">{aiUser.name}</h2>
      </header>
